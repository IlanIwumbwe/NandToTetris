...
std::vector<std::string> Parser::Parse(std::string current_file_path)
{
    std::ifstream infile;
...
void CodeWriter::SetFilePath(std::string current_path){
    current_file_path = current_path;
}

std::string CodeWriter::WriteBootstrapCode(){
    std::string assembly_code = "@256\nD=A\n@SP\nM=D\n" + WriteCall("Sys.init", "0");

    return assembly_code;
}

std::string CodeWriter::WriteArithmetic(const std::string command, const int command_index)
{
...
std::string CodeWriter::WritePushPop(const std::string command, const std::string segment, const std::string index){
    if (command == "push")
    {   
        if (segment == "constant"){
            return "@" + index +"\nD=A\n" + pushToStack;
...
std::string CodeWriter::WriteLabel(std::string label){
    return "(" + current_function + "$" + label + ")";
} 

std::string CodeWriter::WriteGoTo(std::string label){
    return "@" + current_function + "$" + label + "\n0;JMP";
}

std::string CodeWriter::WriteIfGoTo(std::string label){
    return "@SP\nAM=M-1\nD=M\n@" + current_function + "$" + label + "\nD;JNE";
}

std::string CodeWriter::WriteFunction(std::string functionName, std::string nLVars){
...
std::string CodeWriter::WriteCall(std::string functionName, std::string nArgs){
    /*
...
std::string CodeWriter::WriteEndLoop(){
    return "(END)\n@END\n0;JMP";
}
...
void VMtranslator::Translate(std::vector<std::string> file_paths, std::ofstream &output_file){
    // add bootstrap code
