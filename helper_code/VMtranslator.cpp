...
std::string GetOutputPath(std::string &input_path){
    fs::path path(input_path);

    if (fs::is_directory(path)){
        return input_path + "\\" + path.filename().string() + ".asm";   
    }
    else{
        return path.replace_extension(".asm").string();
    }
}
...
int main()
{
    ...
    std::string output_path = GetOutputPath(FILE_PATH);
